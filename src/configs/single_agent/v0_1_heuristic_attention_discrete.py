...
parser = argparse.ArgumentParser(description="Train or evaluate (Heuristic+Attention Discrete).")
...
myTag = "AlphaV0.1_HeuristicAttention_Discrete"
min_gap=2.5
max_accel=2.6
max_decel=4.5
max_speed=55
...
def _build_inflows(tr):
    ...
    inf.add(veh_type="RL", edge="E#L-X", 0.8, depart_lane=0, depart_speed=initial_speed, begin=warmup_steps, color="green")
    return inf
...
def create_flow_env(env_config):
    ...
    return AlphaEnv_v01_HeuristicAttentionDiscrete(env_params=p["env"], sim_params=_s, network=net, simulator=p["simulator"])

register_env("alpha_env_v01_heuristic_attention_discrete", create_flow_env)

def build_config(num_workers=7, render=False):
    return (PPOConfig()
        .environment(env="alpha_env_v01_heuristic_attention_discrete", env_config={"render": render}, disable_env_checking=True)
        .framework("torch")
...
ENV_NAME  = "alpha_env_v01_attention_discrete"
ALGO_NAME = "PPO"

CHECKPOINT_ROOT = os.path.join(
    os.getcwd(),
    "checkpoints/v0_1/attention_discrete",
...
def evaluate(checkpoint_path, num_iterations=20):
    ...
    print(f"\n--- EVAL (Heuristic+Attention Discrete) --- Checkpoint: {checkpoint_path}")
    rews = []
