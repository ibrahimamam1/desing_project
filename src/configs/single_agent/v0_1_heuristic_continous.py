...
#### TRAFFIC RATES — curriculum learning iterates through these
high = 500
medium = 300
low = 150
...
# Build initial flow_params with first traffic rate
net_params = NetParams(
    osm_path=None,
    template=net_file,
    inflows=_build_inflows(traffic_rates[0])
)
...
class TrafficCallbacks(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result: dict, **kwargs) -> None:
        ...
        # Keep only our 5 agent metrics
        if "custom_metrics" in result and isinstance(result["custom_metrics"], dict):
            custom_keep = {
                "collision_mean", "success_mean",
                "avg_speed_mean", "travel_time_mean", "waiting_time_mean",
            }
...
# Checkpoint helpers
# ─────────────────────────────────────────────
ENV_NAME  = "alpha_env_v01_heuristic_continous"
ALGO_NAME = "PPO"

CHECKPOINT_ROOT = os.path.join(
    os.getcwd(),
    "checkpoints/v0_1_heuristic_continous",
...
TENSORBOARD_DIR = os.path.join(os.getcwd(), "tensorboard_logs/v0_1_heuristic_continous")
...
def train():
    ...
    for stage_idx, traffic_rate in enumerate(traffic_rates):
        ...
        for i in range(iters_per_stage):
            ...
            print(f"stage={stage_idx}, iter={global_iter}, reward={current_reward:.3f}, "
    ...
    print("\n--- TRAINING COMPLETE ---")
    print(f"Best Model  → {CHECKPOINT_ROOT}")
    print(f"TensorBoard → {TENSORBOARD_RUN_DIR}")
