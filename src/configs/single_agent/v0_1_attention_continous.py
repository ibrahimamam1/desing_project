...
parser = argparse.ArgumentParser(description="Train or evaluate (Attention).")
...
myTag = "AlphaV0.1_Attention_continous"
min_gap=2.5
max_accel=2.6
max_decel=4.5
max_speed=55
...
def create_flow_env(env_config):
    ...
    return AlphaEnv_v01_Attention(env_params=p["env"], sim_params=_s, network=net, simulator=p["simulator"])

register_env("alpha_env_v01_attention_continous", create_flow_env)

def build_config(num_workers=7, render=False):
    return (PPOConfig()
        .environment(env="alpha_env_v01_attention_continous", env_config={"render": render}, disable_env_checking=True)
        .framework("torch")
...
ENV_NAME  = "alpha_env_v01_att_cont"
ALGO_NAME = "PPO"

CHECKPOINT_ROOT = os.path.join(
    os.getcwd(),
    "checkpoints/v0_1_att_cont",
    f"{ENV_NAME}_{ALGO_NAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
)
FINAL_MODEL_DIR = os.path.join(CHECKPOINT_ROOT, "final")
TENSORBOARD_DIR = os.path.join(os.getcwd(), "tensorboard_logs/v0_1_att_cont")
...
def train():
    ...
    print("\n--- TRAINING COMPLETE ---")
    print(f"Saved Model  → {CHECKPOINT_ROOT}")
    print(f"TensorBoard → {TENSORBOARD_RUN_DIR}")
...
def evaluate(checkpoint_path, num_iterations=20):
    ...
    print(f"\n--- EVAL (Attention) --- Checkpoint: {checkpoint_path}")
    rews = []
