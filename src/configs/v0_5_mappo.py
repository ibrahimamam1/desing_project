...
# Register the MAPPO attention + centralized critic model
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from src.models.attention_centralized_critic_model import AttentionCentralizedCriticModel
ModelCatalog.register_custom_model("attention_centralized_critic", AttentionCentralizedCriticModel)
...
def create_flow_env(env_config):
    ...
    from envs.alpha_env_v05 import AlphaEnv_v05
...
register_env("alpha_env_v05", create_flow_env)

def build_config(num_workers: int = 7, render: bool = False) -> PPOConfig:
        cfg = (
            PPOConfig()
            .environment(env="alpha_env_v05", env_config={"render": render}, disable_env_checking=True)
            .framework("torch")
            .multi_agent(
                policies={"shared_policy": PolicySpec(
                    config={"model": {
                        "custom_model": "attention_centralized_critic",
                        "custom_model_config": {
                            "ego_features": 2,
                            "neighbor_features": 2,
                            "max_neighbors": 5,
...
TENSORBOARD_DIR = os.path.join(os.getcwd(), "tensorboard_logs/v0_5")
