...
# Register the MAPPO centralized critic model (MLP actor + separate critic)
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from src.models.centralized_critic_model import CentralizedCriticModel
...
def create_flow_env(env_config):
    ...
    from envs.alpha_env_v04 import AlphaEnv_v04
...
register_env("alpha_env_v04", create_flow_env)

def build_config(num_workers: int = 7, render: bool = False) -> PPOConfig:
        cfg = (
            PPOConfig()
            .environment(env="alpha_env_v04", env_config={"render": render}, disable_env_checking=True)
            .framework("torch")
            .multi_agent(
...
TENSORBOARD_DIR = os.path.join(os.getcwd(), "tensorboard_logs/v0_4")
