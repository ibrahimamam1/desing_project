...
# Register the custom attention model
...
def create_flow_env(env_config):
    ...
    from envs.alpha_env_v03 import AlphaEnv_v03
...
register_env("alpha_env_v03", create_flow_env)

def build_config(num_workers: int = 7, render: bool = False) -> PPOConfig:
        cfg = (
            PPOConfig()
            .environment(env="alpha_env_v03", env_config={"render": render}, disable_env_checking=True)
            .framework("torch")
            .multi_agent(
...
TENSORBOARD_DIR = os.path.join(os.getcwd(), "tensorboard_logs/v0_3")
