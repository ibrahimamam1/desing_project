...
myTag = "AlphaV0.2_Attention_Discrete"
min_gap       = 2.5
max_accel     = 2.6
max_decel     = 4.5
max_speed     = 55
    ...
    return (PPOConfig()
       ...
       .training(
            ...
            model={"custom_model": "attention_policy", "custom_model_config": {
                "ego_features": 4, "neighbor_features": 6, "max_neighbors": 8,
...
def train():
    ...
    for i in range(num_iterations):
        ...
        total_loss    = learner_stats.get("total_loss", float("nan"))
            ...
            print(f"iter={i}, reward={current_reward:.3f}, "
                  f"loss={total_loss:.4f}, entropy={entropy:.4f}, expl_var={explained_var:.4f}", 
                  flush=True)

        checkpoint_path = algo.save(checkpoint_dir=os.path.join(CHECKPOINT_ROOT, "final"))
        algo.stop()
